...
public class JwtAuthInterceptor implements HandlerInterceptor {
    ...
    private SysUser authenticate(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            writeJson(response, HttpStatus.UNAUTHORIZED.value(), ApiResult.unauthorized("Token无效或已过期"));
            return null;
        }
    }

    private boolean isPublicRequest(HttpServletRequest request, String path) {
        if ("GET".equalsIgnoreCase(request.getMethod()) && isPublicKnowledgeRead(path)) {
            return true;
        }
        return publicPaths.stream().anyMatch(pattern -> pathMatcher.match(pattern, path));
    }

    private boolean isPublicKnowledgeRead(String path) {
        return "/knowledge/article/page".equals(path)
                || pathMatcher.match("/knowledge/article/{id}", path);
    }
    ...
    private boolean requiresAdmin(HttpServletRequest request, String path) {
        ...
        if (pathMatcher.match("/knowledge/article/**", path)) {
